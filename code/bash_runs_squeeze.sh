...
do
	...
	do
		python3 run_experiment_hilbert.py --subject $SUBJECT --model "SQUEEZE" \
		...
		--hilbert_type "none" \
		--model_params "squeeze_params_$i.json" --dropout 0.1 \
...
do
	...
	--augment_jitter 25 --augment_mwrp 0.2 \
	--hilbert_type "none" \
	--model_params "squeeze_params_${BEST_CONFIG}.json" --dropout 0.1 \
